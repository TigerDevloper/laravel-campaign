            ...
            <div class="panel-body" style="padding:20px 0px 0px 0px">
            	...
            	<table class="preview_table table table-hover table-striped table-bordered">
                    ...
                    @foreach($channels as $ch)        
                        <tr rowid="{{$ch->id}}">
                        	<?php 
                        		$temp = explode("-", $ch->tease_start);
                        		$t_start = (int)$temp[2];

                        		$temp = explode("-", $ch->tease_end);
                        		$t_end = (int)$temp[2];

                        		$temp = explode("-", $ch->live_start);
                        		$l_start = (int)$temp[2];

                        		$temp = explode("-", $ch->live_end);
                        		$l_end = (int)$temp[2];

                        		$temp = explode("-", $ch->sustain_start);
                        		$s_start = (int)$temp[2];

                        		$temp = explode("-", $ch->sustain_end);
                        		$s_end = (int)$temp[2];
                        	?>
